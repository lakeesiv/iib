...
Using the `best' TNP model, we will compare the performance of the ConvNP and TNP on fitting sawtooth and GP (using EQ Kernel) data using 1 million parameters. The details technical details of the models of the ConvNP and TNP are given in \autoref{sec:convnp-1d-appendix} and \autoref{sec:tnp-appendix} respectively.
...
In this chapter, we have optimized the hyperparameters of the TNP model and compared the performance of the ConvNP and TNP on 1D datasets. We found that the TNP outperforms the ConvNP on both the sawtooth and GP datasets. The TNP is able to better understand the structure of the data and extrapolate beyond the context set. However, the TNP is more memory intensive than the ConvNP and scales quadratically with the number of context points. The ConvNP scales cubically with the number of dimensions of the data, potentially making the TNP more suitable for high-dimensional data. As such, we will investigate the performance of the models on 2D datasets in the next chapter to see if the difference between the models is exacerbated in higher dimensions.
